...
class Spacemouse(Thread):
    def __init__(self, max_value=500, deadzone=(0,0,0,0,0,0), dtype=np.float32, transmat=1):
        """
        ...
        *----->x right
        ...
        tx_zup_spnav=[np.identity(3, dtype=dtype)]
        tx_zup_spnav.append(np.array([
            [1,0,0],
            [0,0,1],
            [0,1,0]
        ], dtype=dtype))
        ...
        self.tx_zup_spnav=tx_zup_spnav[transmat]
...
def test():
    with Spacemouse(deadzone=0.3,transmat=0) as sm:
        for i in range(2000):
